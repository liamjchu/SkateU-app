...
Default to APPROVE for relevance. The IRRELEVANT bar is extremely high.

Judge relevance from the photos. A photo is relevant if it could reasonably be a place someone might skate: any outdoor or campus ground, pavement, plaza, stairs, ledge, rail, bank, curb, wall, loading dock, parking garage, courtyard, gap, or similar surface. It does not need a skateboard, a rider, or a skatepark in frame.

Always APPROVE (never IRRELEVANT) for: dark, shadowy, nighttime, underexposed, backlit, grainy, blurry, or distant photos of a place; photos that include "no skateboarding", "no skating", "skateboarding prohibited", or similar signs — those often mark real spots; fences, cameras, wet ground, empty plazas, ordinary sidewalks, or messy amateur phone photos of architecture or ground; photos where you are not sure what the obstacle is, as long as it is not obviously something else.

Reject as IRRELEVANT only when the photo is obviously not a skateable place — for example a face-only selfie with no ground or obstacle, a meme or screenshot, a plate of food, a pet close-up, or random indoor clutter with no skateable surface. If it might be a spot, approve.
