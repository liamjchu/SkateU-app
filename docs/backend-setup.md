...
| `OPENAI_API_KEY` | API server | Username, profile bio, spot, and comment moderation, and a safety filter on Help & Support submissions. |
...
3. `supabase/avatars_setup.sql`
4. `supabase/profile_legal_acceptance_setup.sql`
5. `supabase/profile_legal_private_setup.sql`
6. `supabase/spots_setup.sql`
7. `supabase/spots_creator_link.sql`
...
Create a Storage bucket named `avatars` with public read enabled. Profile photos are uploaded only by `/api/profile-avatar` with the service-role key after AI review. Store objects at `avatars/{userId}/{id}.{ext}`. Do not allow client-side uploads.
